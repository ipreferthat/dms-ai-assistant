.pragma library
/**
 * markdown2html.js – Qt/QML markdown to HTML renderer
 * Uses marked.js v1.2.9 (inlined) with a Qt Rich Text-compatible custom renderer.
 *
 * marked v1.2.9  MIT License  https://github.com/markedjs/marked
 */

// ---------------------------------------------------------------------------
// Inline marked.js v1.2.9 (UMD bundle; globalThis shadowed so it uses the
// global-assignment branch and attaches marked to our local object).
// ---------------------------------------------------------------------------
var _md = (function() {
    var globalThis = {};
...
function markdownToHtml(text, colors) {
            ...
            ' style="background-color: ' + c.codeBg + '; margin: 8px 0; table-layout: fixed;">' +
                '<tr><td style="padding: 4px 10px 0 10px;">' +
                    ...
                    ' style="margin: 0; table-layout: fixed;"><tr>' +
                        '<td align="left"  style="padding: 0;"><font size="1" color="#808080">' + lang + '</font></td>' +
                        '<td align="right" style="padding: 0; width: 35px;">' +
                            '<a href="copy://' + b64 + '" style="text-decoration: none;">' +
                            '<font size="1">[Copy]</font></a></td>' +
                    '</tr></table>' +
                '</td></tr>' +
                '<tr><td style="padding: 0 10px 10px 10px;">' +
                    '<pre style="margin: 0; padding: 0; white-space: pre-wrap;"><code>' + escaped + '</code></pre>' +
                '</td></tr>' +
            '</table>'
        );
    ...
    renderer.codespan = function(code) {
        return '<span style="font-family: monospace; background-color: ' +
               c.inlineCodeBg + '; padding: 0 3px;">' + code + '</span>';
    };

    // Blockquote – v1 passes already-rendered inner HTML.
    renderer.blockquote = function(quote) {
        return (
            '<blockquote style="background-color: ' + c.blockquoteBg +
            '; border-left: 4px solid ' + c.blockquoteBorder +
            '; padding: 4px; margin: 8px 0;">' +
            '<font color="#a0a0a0"><i>' + quote + '</i></font></blockquote>'
        );
    };

    // Headings – map h1-h6 to Qt font sizes.
    renderer.heading = function(text, level) {
        var sizes = ["6", "5", "4", "3", "3", "2"];
        var size  = sizes[level - 1] || "2";
        return '<h' + level + ' style="margin-bottom: 8px;">' +
               '<font size="' + size + '">' + text + '</font></h' + level + '>';
    };

    // Images – Qt TextArea cannot display images; show alt text.
