...
Item {
    ...
    function normalizedProfile(id, raw) {
        ...
        if (id === "inception") {
            ...
            profile.customBrowserSearch = (typeof p.customBrowserSearch === "boolean") ? p.customBrowserSearch : !!defaults.customBrowserSearch;
        }
        return profile;
    }
    ...
    function syncLegacySnapshot(activeProfile) {
        ...
        PluginService.savePluginData(pluginId, "customBrowserSearch", !!activeProfile.customBrowserSearch)
    }
    ...
    FileView {
        id: noteFile
        path: Qt.resolvedUrl(notesPath)   // any .txt you want
        blockWrites: false      // we’re only reading, so keep writes blocked // Not blocked rn
        atomicWrites: true

        // Fires when the file finishes loading (or when you call reload())
        onLoaded: {
            notesList = text()               // ← raw string from the file
        }
    ...
    // This will save the New notes file
    function updateNotes(newText) {
        const cleaned = (newText || "").trim();
        notesList = cleaned;
        noteFile.setText(cleaned);
    }

    // This will append the new note
    function appendNote(line) {
        const trimmed = (line || "").trim();
        if (!trimmed) return;
        const updated = notesList ? notesList + "\n" + trimmed : trimmed;
        updateNotes(updated);
    }
